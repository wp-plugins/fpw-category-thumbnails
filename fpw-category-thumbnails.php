...
if ( is_admin() ) {
	...
	$fpw_CT = new fpwCategoryThumbnails( dirname( __FILE__ ), '1.7.0' );
	$o = get_option( 'fpw_category_thumb_opt' );
	if ( is_array( $o ) && $o[ 'fpt' ] ) {
		require_once dirname( __FILE__ ) . '/classes/fpw-post-thumbnails-class.php';
		$fpw_PT = new fpwPostThumbnails( dirname( __FILE__ ), '1.7.0' );
	}
} else {
	//	front end
	...
	$fpw_CT = new fpwCategoryThumbnails( dirname( __FILE__ ), '1.7.0', $hide );
